...
**Справочник:** [auth/AUTH_REFERENCE.md](./auth/AUTH_REFERENCE.md) — §C, legacy-фазы 2–3  
...
`AUTH_API_PATHS` — [`shared/api/authPaths.ts`](../../client/src/shared/api/authPaths.ts).  
Auth HTTP без React — [`pages/Auth/lib/authApi.ts`](../../client/src/pages/Auth/lib/authApi.ts): `postRefresh`, `postLogin`, `postLogout`, `establishSession` → `apiFetch` для `/me`.
...
> Реализация в ветке — [`client/src/pages/Auth/lib/tokenMemory.ts`](../../client/src/pages/Auth/lib/tokenMemory.ts).
...
> Реализация в ветке — [`client/src/shared/api/apiFetch.ts`](../../client/src/shared/api/apiFetch.ts).
...
> Реализация через `authApi`; bootstrap/login/logout — см. [`AuthProvider.tsx`](../../client/src/app/providers/AuthProvider.tsx).
...
### Что уже можно проверить сейчас

- **Фаза A curl** — код готов, можно прогнать таблицу ниже
- **tokenMemory / apiFetch** — тесты можно писать независимо от `AuthProvider`
...
[CURRENT_RELEASE.md](./CURRENT_RELEASE.md) — § Инкремент #3 Auth Forms
