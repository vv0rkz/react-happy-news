...
| ---- | ------------ | ------------- | ------------------- | ---------------- |
| 1 | **D** Backend | `schema`, `authService`, `auth.routes` | curl: register → login → refresh → logout | — (UI ещё нет) |
| 2 | **D** Backend | `authenticate` middleware | curl: Bearer на protected route | — |
| 3 | **A** + §C | `tokenMemory`, `apiFetch`, `AuthProvider` | F5 → auto login по cookie; news через apiFetch | bootstrap cookie + 401→refresh→retry |
| 4 | **A** | `LoginForm`, `RegisterForm`, `authSchemas` | UI login/register | Check-in login |
| 5 | **A** | `ProtectedRoute`, `router` | redirect на `/login`; post-login `state.from` | gate `/favorites` |
| 6 | **A** + §C/§F | изменения в `apiFetch`, `AuthProvider` | logout прерывает in-flight запросы | logout during request |
| 7 | **D** + **A** | `oauthService`, `GoogleButton` | Google OAuth login | — |
...
**Мастер-схема:** D — Backend SOLID (§Архитектура)

| Статус | Фон | Обводка | Текст |
| ------ | --- | ------- | ----- |
| done | нет (default) | тонкая `#64748b` | default |
| **active (WIP)** | `#dce4ef` | жирная `#334155` | `#0f172a` |
| later | `#94a3b8` | тонкая `#64748b` | `#0f172a`; связь `-.->` если узел ещё не подключён |

**После фазы:** curl register → login → refresh → logout

**Сцена timeline B:** — (server-only; UI в фазах 3–4)
...
flowchart TB
  ...
  class routes,service,db phaseActive
  class mw phaseLater
...
**После фазы:** curl с Bearer на protected route → 200 / 401
...
flowchart TB
  ...
  class routes,service,db phaseDone
  class mw phaseActive
...
**Мастер-схема:** A — Module Map (§Архитектура). Zoom: §C — Context vs tokenMemory vs apiFetch
...
**После фазы:** F5 → bootstrap по refresh cookie; GET /api/news через apiFetch с Bearer

**Сцена timeline B:** «Есть cookie?» → POST /refresh; позже GET /news → 401 → /refresh → retry

```mermaid
flowchart TB
  subgraph serverDone [server — фазы 1-2]
  ...
  class srvNote phaseDone
  class apiFetch,AuthProvider,tokenMemory phaseActive
...
**Мастер-схема:** A — Module Map (§Архитектура)
...
**После фазы:** `/login`, `/register` — формы с валидацией; submit → POST /api/auth/*

**Сцена timeline B:** Check-in login — POST /login → boarding pass + Set-Cookie
...
**После фазы:** `/favorites` без login → redirect `/login` с `state.from`; после login — возврат

**Сцена timeline B:** gate `/favorites` — без talon назад к стойке check-in
...
**Мастер-схема:** A — Module Map (§Архитектура). Zoom: §C (logout flow), §F (где хранить что)

| Статус | Фон | Обводка | Текст |
| ------ | --- | ------- | ----- |
| done | нет (default) | тонкая `#64748b` | default |
| **active (WIP)** | `#dce4ef` | жирная `#334155` | `#0f172a` (изменения в существующих узлах) |
| later | `#94a3b8` | тонкая `#64748b` | `#0f172a` |

**После фазы:** logout во время in-flight запроса → abort, token cleared, без retry refresh

**Сцена timeline B:** logout прерывает gate-запрос (isLoggingOut, cancelQueries)

```mermaid
flowchart TB
  subgraph coreMod [core]
    apiFetch["apiFetch.ts<br/>+ isLoggingOut"]
    queryClient["queryClient.ts<br/>+ cancelQueries"]
  end
  subgraph authMod [auth]
    AuthProvider["AuthProvider.tsx<br/>+ abort on logout"]
    tokenMemory["tokenMemory.ts"]
  end
  subgraph appMod [app shell]
    ProtectedRoute["ProtectedRoute.tsx"]
...
**Мастер-схема:** D — Backend SOLID + A — auth-модуль (§Архитектура)
...
**После фазы:** «Sign in with Google» → callback → те же tokens, что при login

**Сцена timeline B:** — (альтернативный check-in через Google)
