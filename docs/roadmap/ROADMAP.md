...
**Полная спека релиза:** [CURRENT_RELEASE.md](./CURRENT_RELEASE.md) — AC, типы, Практика, проверки для всех US v2.2  
...
- Real-time валидация и серверные ошибки (409 email занят)
- После логина — имя пользователя в header

### F2.2.2: Избранное

- Кнопка «В избранное» на карточке
- Страница `/favorites`, синхронизация между устройствами

### F2.2.3: Positivity Tracker

- Страница `/dashboard`: статистика за неделю / месяц, streak, топ тем

### F2.2.5: Protected Routes

- `/favorites` и `/dashboard` только после логина; redirect с `state.from`

## User Stories v2.2

> Детали (Практика, curl, vitest): [CURRENT_RELEASE.md](./CURRENT_RELEASE.md). Не дублировать здесь.

| US | Название | Фаза | Статус | Спека |
| -- | -------- | ---- | ------ | ----- |
| 2.2.1 #1 | Backend Auth | Auth #1 | done | [CURRENT_RELEASE § #1](./CURRENT_RELEASE.md#increment-1) |
| 2.2.1 #2 | Client Session | Auth #2 | active | [CURRENT_RELEASE § #2](./CURRENT_RELEASE.md#increment-2) |
| 2.2.4 | Auth Forms (RHF + Zod) | Auth #3 | pending | [CURRENT_RELEASE](./CURRENT_RELEASE.md) |
| 2.2.5 | Protected Routes + Lazy | Auth #4 | pending | [CURRENT_RELEASE](./CURRENT_RELEASE.md) |
| 2.2.6 | Frontend Security | Auth #5 | pending | [CURRENT_RELEASE](./CURRENT_RELEASE.md) |
| 2.2.13 #6a | Managed Auth ADR | Auth #6a | pending | [US-2.2.13-6a](./auth/increments/US-2.2.13-6a-adr.md) |
| 2.2.13 #6b | Managed Auth Migration | Auth #6b | pending | [US-2.2.13-6b](./auth/increments/US-2.2.13-6b-migrate.md) |
| 2.2.2 | Избранное | Персонализация | pending | [CURRENT_RELEASE § 2.2.2](./CURRENT_RELEASE.md#us-2-2-2) |
| 2.2.3 | Positivity Tracker | Персонализация | pending | [CURRENT_RELEASE § 2.2.3](./CURRENT_RELEASE.md#us-2-2-3) |
| 2.2.9 | GDPR / Right to delete | Хвост | pending | [CURRENT_RELEASE](./CURRENT_RELEASE.md) |
| 2.2.7 | Email verification | — | superseded | Заменён 2.2.13 |
| 2.2.8 | Password reset | — | superseded | Заменён 2.2.13 |
| 2.2.10 | OAuth Google (passport) | — | cancelled | Заменён 2.2.13 |
| 2.2.11 | Сравнение хранилищ | optional | — | После auth-трека |
| 2.2.12 | Тёмная / светлая тема | optional | — | localStorage |
...
| 6a | 2.2.13 | Managed Auth — ADR + выбор провайдера |
| 6b | 2.2.13 | Managed Auth — миграция (Clerk/Auth0) |

> **#6** заменяет 2.2.10 (passport). ADR: [auth/MANAGED_AUTH_ADR.md](./auth/MANAGED_AUTH_ADR.md). **Предусловие #6:** #2–#5 ✅.
...
│   ├── favorites.routes.ts    ← CRUD избранного
...
client/src/
├── shared/api/
│   └── apiFetch.ts            ← fetch + 401 auto-refresh
├── pages/Auth/                ← LoginForm, RegisterForm, tokenMemory, useAuth
├── pages/Favorites/
├── pages/Dashboard/
...
| FQ37 | Context vs Redux            | Auth (Context + tokenMemory), News (TanStack Query) |
