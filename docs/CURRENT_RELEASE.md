...
**Статус:** `in progress` (US 2.0.1 ✅, US 2.0.3 ✅, US 2.0.5 ✅ → US 2.0.7 next)
...
- [x] CORS-заголовки настроены (localhost:5173 + 127.0.0.1:5173, через CORS_ORIGIN env)
- [x] Preflight (OPTIONS) обрабатывается cors middleware
- [ ] В проде — CORS ограничен конкретным доменом (v2.5)

### US 2.0.5: Swagger-документация + OpenAPI codegen — ✅ DONE

- [x] Swagger UI на `/api/docs`
- [x] Все endpoints описаны через реальные Zod-схемы (zod-to-openapi, без JSDoc)
- [x] OpenAPI spec генерируется из Zod → единственный источник правды
- [x] `openapi-typescript` генерирует типы клиента из spec → убраны ручные типы
- [x] `pnpm gen:openapi` / `pnpm gen:openapi:sync` — команды для обновления типов
- [x] Закрывает Q88: "OpenAPI spec → codegen" на практике
